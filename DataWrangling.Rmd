...
- add hidden code for Eerkens Table 2 and Table 3 
- add revealable code? **unilur**?
- make sure structure of each section is 'Demo' with Drennan, then 'Putting it into practice' with Eerkens. Or re-order; all demo, then all 'putting it into practice'?
...
Because this is such a common problem, data *cleaning* and data *wrangling* are fantastically useful skills. The former comprises everything from making column names machine-readable to removing empty rows to standardizing names, while the latter commonly involves renaming, recoding, summarizing, and reshaping data.
...
What we want to do is tidy these tables up, and combine them into a single table that includes a variable for site (Table 1 is material from Sherwin Summit, Table 2 from Mohawk Valley, and Table 3 from Bone Cave). That desire leads us to...

# Cleaning data

-   standardize no data value to NA
-   flesh out column names and make sure they match between tables
-   remove marginal totals and then join, allowing for the fact that not every site has material from the same sources [data wrangling]
-   exploratory visualization as payoff

We'll do this the **tidyverse** [`r emo::ji("face with rolling eyes")`] way, using `pipes` (`%>%`), since we're going to use the **dplyr** package and most of the examples you'll find do so. Note that even using pipes, if you don't write the result to an object (often overwriting the one that you feed into the pipe), it will be ephemeral. So, `drennan6.1b <- drennan6.1b %>%` is more likely to be useful than just `drennan6.1b %>%`. 
...
#mutate can also be used to create new columns, as assigning to `$` would
#but remember that piping is ephemeral if you're not writing to an object
...
We'll start by defining the no-data values. I find the **dplyr** syntax here annoying and almost illegible...but it's undeniably effective.

```{r defineNA}

eerk_tab1 <- eerk_tab1 %>% mutate(across(where(is.character), ~na_if(., "-")))

### or with replace()? 
```

```{r echo = F}
eerk_tab2 <- eerk_tab2 %>% mutate(across(where(is.character), ~na_if(., "-")))

eerk_tab3 <- eerk_tab3 %>% mutate(across(where(is.character), ~na_if(., "-")))
```

We can "recode" to deal with typos (or any other values that we'd like to change). This used to be handled in **dplyr** by a function called - sensibly - `recode()`...but that has been replaced by one called `case_match()`. Although you can still use `recode()`, we'll try to keep up with the times by using `case_match()`.
...
As you'll have noticed above, column names with punctuation can be a hassle (need to be enclosed in tickmarks [`]). 
The **janitor** package ("simple little tools for examining and cleaning dirty data") streamlines the standardization of these names (the `case=` argument controls whether you'll get "Source Artifact", "source_artifact", or "SourceArtifact", etc.). 
...
One common task to is integrate information that has been stored inconveniently. In the case of Table 6.1b, there's information stored in the second sheet that we might like to include in our table. Because there's not much of it, we can do this manually, by recoding. We'll also explore automating the process, which you would want to do with there were 30 rows rather than 3. 

## `case_when()`
First we'll use **dplyr**'s `case_when()` function, which is effectively a version of `if_else()`. We'll combine it with `mutate()` to make a new column, and then populate that column depending on what the value of another column in the same row is. What we're doing in R working out way down the rows of Table 6.1b, checking the site, then looking at Table 6.1b Sheet 2 to see what period that site belongs to, and filling that information in a new column.
...
drennan6.1b %>% group_by(Site) %>% summarize(Incised = sum(Incised), Unincised = sum(Unincised),
                                             Units = length(Unit))
...
```{r}
eerk_comb %>% group_by(source_artifact) %>% summarize(source_diversity = mean(sources_represented))
```
...
The `pivot` vocabulary and syntax are often incomprehensible, but it's possible to follow the examples in the vignette that accompanies the package (try `vignette("pivot")`).
...
Now that we have the Eerkens et al. 2007 data cleaned and in the form that we'd like, what do we want to *do* with it? For starters, let's compare the diversity of sources for each artifact class across the different sites. 
...
eerk_comb_long <- eerk_comb %>% pivot_longer(cols = !c(site, source_artifact, sources_represented), 
                           names_to = "Source",
                           values_to = "Count") %>%
...
#a lot
mypalette <- colorRampPalette(brewer.pal(12, "Set3"))(29)
